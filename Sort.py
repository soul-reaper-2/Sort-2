...
# --------------- TELEGRAM BOT SETUP ------------- #
app = Client("sortbot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
async def raw_handler(client, update, users, chats):
    print(f"[RAW UPDATE] {type(update).__name__}: {update}")

@app.on_message(filters.private)
async def log_all(client, message):
    print(f"[MSG] From: {message.from_user.id} | Text: {message.text or '<media>'}")
...
async def startup():
    await init_db()
    me = await app.get_me()
    print(f"✅ Sort-bot is running as @{me.username} (ID: {me.id})")
