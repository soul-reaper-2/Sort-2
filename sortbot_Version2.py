...
# --------------------------------------------------------------------------
BOT_TOKEN = os.getenv("BOT_TOKEN", "")  # Or hardcode here
OWNER_USER_ID = int(os.getenv("OWNER_USER_ID", "0"))  # Or hardcode here
DB_FILE = "sortbot.db"
...
# Used by in-memory channel selection (per user)
_pick_state = dict()
...
CREATE TABLE IF NOT EXISTS user_queue (
    user_id INTEGER, file_id TEXT, chat_id INTEGER, msg_id INTEGER,
    file_type TEXT, filename TEXT, title TEXT, season TEXT, episode TEXT, quality TEXT, audio TEXT, ts INTEGER
);
...
# --------------------------------------------------------------------------
# USER SORT FORM, HEADERS, FOOTERS
# --------------------------------------------------------------------------
...
def parse_filename(filename: str):
    # Remove extension, release groups, channel tags
    ...
    if not sm: sm = re.search(r"\bS(\d+)\b", name, re.I)
    if sm: season = str(int(sm.group(1)))
    # Episode
    ...
    if em:
        ...
        if em: episode = em.group(1)
        else:  # pick standalone number, but not in known patterns (if not quality, season, etc.)
    ...
    tmp = re.sub(r"\s+", " ", tmp)
    title = tmp.strip()
    # Skip OVAs, Movies etc for episode
...
async def start(client, message):
    await message.reply("Welcome to Sort-bot! Use /help.")
...
async def help_cb(client, cq):
    ...
    if cat == "queue":
        ...
        text = (
            ...
            f"Allowed: {', '.join(ALLOWED_FORM_FIELDS)}"
        )
        ...
        return
    kbs = [[InlineKeyboardButton("Back", callback_data="help_back")], [InlineKeyboardButton("Cancel", callback_data="help_cancel")]]
...
async def help_close(client, cq): await cq.message.delete()

# -------- ADMIN AND CHANNEL COMMANDS -------- #
...
async def show_channel_picker(client, msg, chas, segs, skipped):
    ...
    _pick_state[msg.from_user.id] = {"sel": set(), "chas": chas, "segs": segs, "skipped": skipped}

@app.on_callback_query(filters.regex("^pick_"))
async def pick_cb(client, cq):
    user_id = cq.from_user.id
    state = _pick_state.get(user_id)
    if not state:
